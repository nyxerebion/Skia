...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($email)) {
        ...
        $stmt = $pdo->prepare("
            SELECT id, username, last_reset_request
            FROM users 
            WHERE email = ?
        ");
        $stmt->execute([$email]);
        ...
        if (!$user) {
            ...
            // Check cooldown - FIX: stop execution
            if ($user['last_reset_request'] && strtotime($user['last_reset_request']) > time() - 1800) {
                $error = 'Please wait 30 minutes before requesting again.';
            } else {
                // Generate token
                ...
                $resetLink = SITE_URL . '/security/reset-password.php?token=' . $token;
                $body = passwordResetEmail($user['username'], $resetLink);

                $sent = sendEmail($email, 'Reset Your Password - Skia', $body);

                if ($sent) {
                    header('Location: reset-sent.php?email=' . urlencode($email));
                    exit;
                } else {
